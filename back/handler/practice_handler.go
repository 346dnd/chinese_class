...
func (h *PracticeHandler) SubmitPractice(c *gin.Context) {
	...
	resp, err := h.practiceService.SubmitPractice(req, studentId)
	if err != nil {
...
func (h *PracticeHandler) SubmitReadAloud(c *gin.Context) {
	studentId := c.GetHeader("X-Student-Id")
	if studentId == "" {
		response.JSONError(c, response.CodeInvalidParams, "缺少学生ID")
		return
	...
	var req model.ReadAloudRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.JSONError(c, response.CodeInvalidParams, "请求参数错误")
		return
	}
	...
	resp, err := h.practiceService.SubmitReadAloud(req, studentId)
	if err != nil {
		response.JSONError(c, response.CodeInternalError, err.Error())
		return
	}
