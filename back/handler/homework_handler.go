...
func (h *HomeworkHandler) Recommend(c *gin.Context) {
	...
	var req model.HomeworkRecommendRequest
	...
	resp, err := h.homeworkService.Recommend(req, studentId)
	if err != nil {
...
func (h *HomeworkHandler) Submit(c *gin.Context) {
	...
	var req model.HomeworkSubmitRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.JSONError(c, response.CodeInvalidParams, "请求参数错误")
		return
	}
	...
	resp, err := h.homeworkService.Submit(req, studentId)
	if err != nil {
		response.JSONError(c, response.CodeInternalError, err.Error())
		return
	}
